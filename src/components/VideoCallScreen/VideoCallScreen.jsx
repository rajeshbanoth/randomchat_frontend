...
//       // On desktop: side by side (horizontal grid)
...
//     case 'grid-vertical':
//       // On desktop: stacked vertically
//       // On mobile: side by side (but vertical orientation)
//       return (
...
//     case 'side-by-side':
//       // Fixed 70/30 ratio on desktop, vertical split on mobile
//       return (
...
//     case 'stack':
//       // Stacked layout - remote full, local overlay
//       return (
...
//     case 'cinema':
//       // Cinema mode - remote with black bars, local in corner
//       return (
...
//     case 'speaker-view':
//       // Speaker view - main speaker large, other small
//       return (
...
//     case 'focus-remote':
//       // Focus on remote, local as small PIP
//       return (
...
//     case 'focus-local':
//       // Focus on local, remote as small PIP
//       return (
...
import { initializeLocalStreamFn } from './functions/webrtc/localStream';
import { forceStreamSyncFn } from './functions/webrtc/forceStreamSync';
// ==================== FIXED PLACEHOLDER STREAM FUNCTION ====================
const createPlaceholderStream = () => {
  console.log('🎬 Creating placeholder video stream...');
  ...
  try {
    ...
    // Animation loop - only draw, no recapture
    let angle = 0;
    const animate = () => {
      // Clear canvas
      ctx.fillStyle = '#1f2937';
...
const VideoChatScreen = () => {
  ...
  useEffect(() => {
    const checkScreenSize = () => {
      ...
      // Auto-select layout based on screen size
      if (mobile) {
        setVideoLayout('grid-vertical'); // Vertical grid for mobile
      } else {
        setVideoLayout('side-by-side'); // Side-by-side for PC
      }
    };
  ...
  // ==================== CRITICAL FIX: Enhanced layout change handler ====================
  const handleLayoutChange = (newLayout, animate = true) => {
    console.log(`🔄 Changing layout from ${videoLayout} to ${newLayout}`);
  ...
  // ==================== FIXED: Reattach streams on layout change, stream change, screen share ====================
  useEffect(() => {
    console.log('🔄 Reattaching streams due to layout/stream change');

    if (remoteVideoRef.current && remoteStreamRef.current) {
      remoteVideoRef.current.srcObject = remoteStreamRef.current;
      remoteVideoRef.current.play().catch(e => console.error('Remote play error:', e.message));
    }

    if (localVideoRef.current) {
      const currentLocalStream = isScreenSharing && screenStreamRef.current 
        ? screenStreamRef.current 
        : localStreamRef.current;
      if (currentLocalStream) {
        localVideoRef.current.srcObject = currentLocalStream;
        localVideoRef.current.play().catch(e => console.error('Local play error:', e.message));
      }
    }
  }, [videoLayout, remoteStream, localStream, isScreenSharing]);
  // ==================== WEBRTC FUNCTIONS ====================
  // Instantiate the functions from factories
  const processQueuedIceCandidates = useCallback(
    processQueuedIceCandidatesFactory({
  ...
  // Enhanced createAndUsePlaceholder function for manual trigger
  const createAndUsePlaceholder = () => {
    console.log('🔄 Manually creating placeholder stream...');

    // Stop existing stream if any
    if (localStreamRef.current) {
      if (localStreamRef.current._cleanup) {
...
const renderVideoLayout = () => {
  ...
  const containerClass = `absolute inset-0 transition-all duration-500 ease-in-out ${
    isChangingLayout ? 'opacity-90 scale-95 blur-sm' : 'opacity-100 scale-100'
  }`;
  ...
  switch(videoLayout) {
    case 'grid-horizontal':
      ...
      return (
        <div ref={videoContainerRef} className={`flex ${isMobile ? 'flex-col' : 'flex-row'} h-full gap-2 p-2 ${layoutAnimations['grid-horizontal']} ${containerClass}`}>
          {/* Remote Video */}
          <div className={`${isMobile ? 'h-1/2' : 'flex-1'} relative rounded-2xl overflow-hidden bg-black`}>
            <video {...remoteVideoProps} />
            {renderVideoOverlay('remote')}
          </div>

          {/* Local Video */}
          <div className={`${isMobile ? 'h-1/2' : 'flex-1'} relative rounded-2xl overflow-hidden border-2 ${usingPlaceholder ? 'border-purple-500/50' : 'border-gray-700/50'} bg-black`}>
            <video {...localVideoProps} />
            {renderVideoOverlay('local')}
          </div>
        </div>
    ...
    case 'grid-vertical':
      ...
      return (
        <div ref={videoContainerRef} className={`flex ${isMobile ? 'flex-row' : 'flex-col'} h-full gap-2 p-2 ${layoutAnimations['grid-vertical']} ${containerClass}`}>
          {/* Remote Video */}
          <div className={`${isMobile ? 'w-1/2' : 'h-1/2'} relative rounded-2xl overflow-hidden bg-black`}>
            <video {...remoteVideoProps} />
            {renderVideoOverlay('remote')}
          </div>

          {/* Local Video */}
          <div className={`${isMobile ? 'w-1/2' : 'h-1/2'} relative rounded-2xl overflow-hidden border-2 ${usingPlaceholder ? 'border-purple-500/50' : 'border-gray-700/50'} bg-black`}>
            <video {...localVideoProps} />
            {renderVideoOverlay('local')}
          </div>
        </div>
    ...
    case 'side-by-side':
      ...
      return (
        <div ref={videoContainerRef} className={`flex ${isMobile ? 'flex-col' : 'flex-row'} h-full gap-3 p-3 ${layoutAnimations['side-by-side']} ${containerClass}`}>
          {/* Remote Video - 70% on desktop, 2/3 on mobile */}
          <div className={`${isMobile ? 'h-2/3' : 'w-[70%]'} relative rounded-2xl overflow-hidden bg-black`}>
            <video {...remoteVideoProps} />
            {renderVideoOverlay('remote')}
          </div>

          {/* Local Video - 30% on desktop, 1/3 on mobile */}
          <div className={`${isMobile ? 'h-1/3' : 'w-[30%]'} relative rounded-2xl overflow-hidden border-2 ${usingPlaceholder ? 'border-purple-500/50' : 'border-gray-700/50'} bg-black`}>
            <video {...localVideoProps} />
            {renderVideoOverlay('local')}
          </div>
        </div>
    ...
    case 'stack':
      ...
      return (
        <div ref={videoContainerRef} className={`relative h-full ${layoutAnimations.stack} ${containerClass}`}>
          ...
          {/* Local Video - Corner Overlay */}
          <div className={`absolute ${isMobile ? 'bottom-4 left-4 w-32 h-24' : 'bottom-8 left-8 w-64 h-48'} rounded-2xl overflow-hidden border-2 ${usingPlaceholder ? 'border-purple-500/50' : 'border-gray-700/50'} bg-black`}>
            <video {...localVideoProps} />
            {renderVideoOverlay('local', true)}
          </div>
        </div>
    ...
    case 'cinema':
      ...
      return (
        <div ref={videoContainerRef} className={`relative h-full ${layoutAnimations.cinema} ${containerClass}`}>
          ...
          {/* Local Video - Small Corner */}
          <div className={`absolute ${isMobile ? 'top-4 right-4 w-32 h-24' : 'top-8 right-8 w-48 h-36'} rounded-xl overflow-hidden border-2 ${usingPlaceholder ? 'border-purple-500/50' : 'border-gray-700/50'} bg-black`}>
            <video {...localVideoProps} />
            {renderVideoOverlay('local', true)}
          </div>
        </div>
    ...
    case 'speaker-view':
      ...
      return (
        <div ref={videoContainerRef} className={`relative h-full ${layoutAnimations['speaker-view']} ${containerClass}`}>
          {/* Main Speaker (Remote) - Larger area */}
          <div className={`absolute ${isMobile ? 'inset-x-0 top-0 h-3/4' : 'inset-x-0 top-0 h-3/4'} bg-black`}>
            <video {...remoteVideoProps} />
            {renderVideoOverlay('remote')}
          </div>

          {/* Local Video - Smaller at bottom */}
          <div className={`absolute ${isMobile ? 'bottom-4 left-4 right-4 h-32' : 'bottom-8 left-1/3 right-1/3 h-1/4'} rounded-xl overflow-hidden border-2 ${usingPlaceholder ? 'border-purple-500/50' : 'border-gray-700/50'} bg-black`}>
            <video {...localVideoProps} />
            {renderVideoOverlay('local', true)}
          </div>
        </div>
    ...
    case 'focus-local':
      ...
      return (
        <div ref={videoContainerRef} className={`relative h-full ${layoutAnimations['focus-local']} ${containerClass}`}>
          ...
          {/* Remote Video - Small PIP */}
          <div className={`absolute ${isMobile ? 'top-4 left-4 w-32 h-24' : 'top-8 left-8 w-48 h-36'} rounded-xl overflow-hidden border-2 border-gray-700/50 bg-black`}>
            <video {...remoteVideoProps} />
            {renderVideoOverlay('remote', true)}
          </div>
        </div>
    ...
    default:
      ...
      return (
        <div ref={videoContainerRef} className={`relative h-full ${layoutAnimations.pip} ${containerClass}`}>
          ...
          {/* Local Video - PIP */}
          <div className={`absolute ${isMobile ? 'top-4 right-4 w-40 h-30' : 'top-8 right-8 w-64 h-48'} rounded-2xl overflow-hidden border-2 ${usingPlaceholder ? 'border-purple-500/50' : 'border-gray-700/50'} bg-black`}>
            <video {...localVideoProps} />
            {renderVideoOverlay('local', true)}
          </div>
        </div>
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="pt-4 border-t border-gray-800">
              ...
              <div className="space-y-2">
                ...
                <button
                  onClick={retryLocalStream}
                  className="w-full px-3 py-2 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 hover:from-blue-500/30 to-cyan-500/30 rounded-lg text-sm transition-all duration-300 backdrop-blur-sm border border-blue-500/30 hover:scale-[1.02]"
                >
                  Reinitialize Camera
                </button>
                ...
                <button
                  onClick={createAndUsePlaceholder}
                  className="w-full px-3 py-2 bg-gradient-to-r from-purple-500/20 to-pink-500/20 hover:from-purple-500/30 to-pink-500/30 rounded-lg text-sm transition-all duration-300 backdrop-blur-sm border border-purple-500/30 hover:scale-[1.02]"
                >
                  {usingPlaceholder ? 'Refresh Placeholder' : 'Use Placeholder Video'}
                </button>
              </div>
            ...
            <div className="pt-4 border-t border-gray-800">
              <button
                ...
                className="w-full px-3 py-2 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 hover:from-yellow-500/30 to-orange-500/30 rounded-lg text-sm transition-all duration-300 backdrop-blur-sm border border-yellow-500/30 hover:scale-[1.02]"
              >
                Force Stream Sync
              </button>
            </div>

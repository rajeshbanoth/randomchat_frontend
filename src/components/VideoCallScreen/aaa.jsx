...
const VideoChatScreen = () => {
  ...
  console.log('🎥 Rendering VideoChatScreen with partner:', partner);
  // Refs
  const localVideoRef = useRef(null);
  const remoteVideoRef = useRef(null);
  const peerConnectionRef = useRef(null);
  ...
  const handleVideoMatchReady = useCallback((data) => {
    ...
    console.log('🔍 Detailed match data:', {
      ...
      dataPartnerId: data.partnerId,
      dataTimestamp: data.timestamp,
      partnerDetails: partner ? { id: partner.id, _id: partner._id, partnerId: partner.partnerId } : 'No partner object'
    });
    ...
    if (partner && debugForcePartnerUpdate) {
      ...
      console.log('🔄 Forcing partner update:', {
        originalPartner: partner,
        updatedPartner
      });
      debugForcePartnerUpdate(updatedPartner);
    }
  ...
  const handleWebRTCOffer = useCallback(async (data) => {
    ...
    console.log('🔍 Detailed offer SDP:', data.sdp ? JSON.stringify(data.sdp, null, 2) : 'No SDP');
    ...
    if (hasLocalOffer) {
      ...
      if (weShouldBeCaller) {
        ...
        if (pc.localDescription) {
          console.log('🔍 Re-sending offer SDP:', JSON.stringify(pc.localDescription, null, 2));
          sendWebRTCOffer({
            to: data.from,
            sdp: pc.localDescription,
            callId: data.callId || callInfo.callId,
        ...
        await pc.setLocalDescription(null);
        console.log('✅ Local description rolled back to null');
      }
    }
    ...
    try {
      ...
      setCallInfo(prev => {
        ...
        console.log('🔄 Updating callInfo:', { previous: prev, updated });
        return updated;
      });
      ...
      console.log('🔍 Created answer SDP:', JSON.stringify(answer, null, 2));
  ...
  const handleWebRTCAnswer = useCallback(async (data) => {
    ...
    console.log('🔍 Detailed answer SDP:', data.sdp ? JSON.stringify(data.sdp, null, 2) : 'No SDP');
    
    if (data.roomId && !callInfo.roomId) {
      console.log('🔁 Updating callInfo with roomId from answer:', data.roomId);
      setCallInfo(prev => {
        const updated = { ...prev, roomId: data.roomId };
        console.log('🔄 callInfo update:', { previous: prev, updated });
        return updated;
      });
    }
    ...
    if (signalingState === 'stable' && remoteDescType === 'answer') {
      ...
      if (localDescType === 'offer' && !callInfo.isCaller) {
        console.log('🔄 Correcting role: we are caller (we sent offer)');
        setCallInfo(prev => {
          const updated = { ...prev, isCaller: true };
          console.log('🔄 Role update:', { previous: prev, updated });
          return updated;
        });
      }
      ...
      setTimeout(() => {
        ...
        receivers.forEach((rec, idx) => {
          if (rec.track) {
            console.log(`  Receiver ${idx}: ${rec.track.kind} - ${rec.track.readyState} - enabled: ${rec.track.enabled}`);
          } else {
    ...
    if (signalingState === 'have-local-offer') {
      ...
      if (localDescType === 'answer') {
        console.log('🔄 Re-sending our answer...');
        console.log('🔍 Re-sending answer SDP:', JSON.stringify(pc.localDescription, null, 2));
        sendWebRTCAnswer({
          to: data.from,
          sdp: pc.localDescription,
          callId: data.callId || callInfo.callId,
  ...
  const handleWebRTCIceCandidate = useCallback(async (data) => {
    ...
    console.log('🔍 Full ICE candidate details:', data.candidate ? JSON.stringify(data.candidate, null, 2) : 'No candidate');
    ...
    if (peerConnectionRef.current && data.candidate) {
      try {
        if (!peerConnectionRef.current.remoteDescription) {
          ...
          console.log('📥 Queued candidates count:', queuedIceCandidatesRef.current.length);
  ...
  const handleWebRTCEnd = useCallback((data) => {
    console.log('📵 WebRTC call ended:', data);
    console.log('🔍 End event details:', JSON.stringify(data, null, 2));
    addNotification('Partner ended the video call', 'info');
    handleDisconnect();
  }, [addNotification]);
  ...
  useEffect(() => {
    ...
    // Try to initialize with real camera first, fallback to placeholder
    const initStream = async () => {
      console.log('📹 Initializing local stream (real camera attempt)');
      const success = await initializeLocalStream(false);
      console.log('📊 Stream init result:', { success });
      if (!success) {
        console.log('🔄 Falling back to placeholder after mount');
        const fallbackSuccess = await initializeLocalStream(true);
        console.log('📊 Fallback stream init result:', { fallbackSuccess });
      }
    };
    ...
    console.log('⏱️ Setting up call timer');
    callTimerRef.current = setInterval(() => {
      setCallDuration(prev => {
        const newDuration = prev + 1;
        console.log('⏱️ Call duration updated:', newDuration);
        return newDuration;
      });
    }, 1000);
    ...
    const hideControls = () => {
      if (controlsTimeoutRef.current) {
        console.log('🕒 Clearing existing controls timeout');
        clearTimeout(controlsTimeoutRef.current);
      }
      controlsTimeoutRef.current = setTimeout(() => {
    ...
    const handleMouseMove = () => {
      console.log('🖱️ Mouse move detected, showing controls');
      setShowControls(true);
      hideControls();
    };
    
    console.log('🖱️ Adding mouse move listener');
    window.addEventListener('mousemove', handleMouseMove);
    ...
    return () => {
      ...
      if (controlsTimeoutRef.current) {
        console.log('🕒 Clearing controls timeout in cleanup');
        clearTimeout(controlsTimeoutRef.current);
      }
      if (callTimerRef.current) {
        console.log('⏱️ Clearing call timer in cleanup');
        clearInterval(callTimerRef.current);
      }
      if (statsIntervalRef.current) {
        console.log('📊 Clearing stats interval in cleanup');
        clearInterval(statsIntervalRef.current);
      }
      if (retryTimeoutRef.current) {
        console.log('🔄 Clearing retry timeout in cleanup');
  ...
  const debugStreamInfo = () => {
    ...
    console.log('🔍 Checking WebRTC init conditions:', {
      ...
      notInitialized: !callInfo.initialized,
      shouldInitialize
    });
    
    if (shouldInitialize) {
      console.log('🚀 Conditions met for WebRTC initialization');
      console.log('🔍 Current callInfo:', JSON.stringify(callInfo, null, 2));
      initializeWebRTCConnection();
      setCallInfo(prev => {
        const updated = { ...prev, initialized: true };
        console.log('🔄 Updating callInfo initialized:', { previous: prev, updated });
        return updated;
      });
    }
  }, [localStreamRef.current, partner, callInfo, initializeWebRTCConnection]);
